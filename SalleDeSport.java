...
public class SalleDeSport {
    ...
    public double chiffreAffairesPrestations() {
        double ca = 0;
        for (Adherent a : adherents) {
            ca += a.depensesTotales();
        }
        return ca;
    }

    public Adherent trouverAdherent(int id) throws AdherentNonTrouveException {
        for (Adherent a : adherents) {
            if (a.getId() == id) {
                return a;
            }
        }
        throw new AdherentNonTrouveException("Aucun adhérent trouvé avec l'ID: " + id);
    }
}